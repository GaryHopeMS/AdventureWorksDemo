...
namespace modeling_demos
{
    class Program
    {
        ...
        private static readonly string serverlessURI = config["ServerlessURI"];
        private static readonly string serverlessKey = config["ServerlessKey"];
        private static readonly string provisionedURI = config["ProvisionedURI"];
        private static readonly string provisionedKey = config["ProvisionedKey"];

        private static readonly CosmosClient serverlessClient = new CosmosClient(serverlessURI, serverlessKey);
        private static readonly CosmosClient provisionedClient = new CosmosClient(provisionedURI, provisionedKey);
        ...
        public static async Task Main(string[] args)
        {
            ...
            while (exit == false)
            {
                ...
                {
                    Console.Clear();
                    await CreateServerlessDatabasesAndContainers();
                }
                else if (result.KeyChar == 'b')
                {
                    Console.Clear();
                    await UploadServerlessData();
                }
                else if (result.KeyChar == 'v')
                {
                    Console.Clear();
                    await DeleteServerlessData();
                }
                else if (result.KeyChar == 'w')
                {
                    Console.Clear();
                    await DeleteProvisionedData();
                }
                if (result.KeyChar == 'c')
                {
                    Console.Clear();
                    await CreateProvisionedDatabasesAndContainers();
                }
        ...
        private static async Task DeleteServerlessData()
        {
            Database serverlessDatabase = serverlessClient.GetDatabase("AdventureWorks");
            DatabaseResponse databaseResponse = await serverlessDatabase.DeleteAsync();
            Console.WriteLine($"  Database deleted");
        }

        private static async Task CreateServerlessDatabasesAndContainers()
        {
            Database serverlessDatabase = await serverlessClient.CreateDatabaseAsync("AdventureWorks");
            Console.WriteLine($"  {serverlessDatabase.Id} created");
            Container productContainer = await serverlessDatabase.CreateContainerAsync("Product", "/categoryId");
            Console.WriteLine($"  {productContainer.Id} created");
            Container customerContainer = await serverlessDatabase.CreateContainerAsync("Customer", "/id");
            Console.WriteLine($"  {customerContainer.Id} created");
            Container salesOrderContainer = await serverlessDatabase.CreateContainerAsync("SalesOrder", "/customerId");
            Console.WriteLine($"  {salesOrderContainer.Id} created");
            Console.WriteLine($"Press any key to continue");
            Console.Read();
        }

        private static async Task CreateProvisionedDatabasesAndContainers()
        {
            Database goodDatabase = await provisionedClient.CreateDatabaseAsync("AdventureWorksGood");
            Console.WriteLine($"  {goodDatabase.Id} created");
            Database badDatabase = await provisionedClient.CreateDatabaseAsync("AdventureWorksBad");
            Console.WriteLine($"  {badDatabase.Id} created");

            ThroughputProperties productThroughput = ThroughputProperties.CreateAutoscaleThroughput(4000);
            ThroughputProperties customerThroughput = ThroughputProperties.CreateAutoscaleThroughput(100000);

            Container goodProductContainer = await goodDatabase.CreateContainerAsync(new ContainerProperties( "Product", "/categoryId") , productThroughput);
            Console.WriteLine($"  {goodProductContainer.Id} created");
            Container goodCustomerContainer = await goodDatabase.CreateContainerAsync(new ContainerProperties("Customer", "/customerId"), customerThroughput);
            Console.WriteLine($"  {goodCustomerContainer.Id} created");

            Container badProductContainer = await badDatabase.CreateContainerAsync(new ContainerProperties("Product", "/categoryId"), productThroughput);
            Console.WriteLine($"  {badProductContainer.Id} created");
            Container badCustomerContainer = await badDatabase.CreateContainerAsync(new ContainerProperties("Customer", "/orderDate"), customerThroughput);
            Console.WriteLine($"  {badCustomerContainer.Id} created");

            Console.WriteLine($"Press any key to continue");
            Console.Read();
        }

        private static async Task DeleteProvisionedData()
        {
            ...
            DatabaseResponse goodDatabaseResponse = await goodDatabase.DeleteAsync();
            Console.WriteLine($"  Database deleted");
            DatabaseResponse badDatabaseResponse =  await badDatabase.DeleteAsync();
            Console.WriteLine($"  Database deleted");
        }

        private static async Task UploadProvisionedData()
        {
            Database goodDatabase = provisionedClient.GetDatabase("AdventureWorksGood");
            Database badDatabase = provisionedClient.GetDatabase("AdventureWorksBad");
            
            string sourceDatabaseName = "database-v4";
            ...
            Container goodProductContainer = goodDatabase.GetContainer("Product");
            await Deployment.LoadContainerFromFile(goodProductContainer, folder + Path.DirectorySeparatorChar + "Product");
            Container goodCustomerContainer = goodDatabase.GetContainer("Customer");
            await Deployment.LoadContainerFromFile(goodCustomerContainer, folder + Path.DirectorySeparatorChar + "Customer");

            Container badProductContainer = badDatabase.GetContainer("Product");
            await Deployment.LoadContainerFromFile(badProductContainer, folder + Path.DirectorySeparatorChar + "Product");
            ...
            private static async Task UploadServerlessData()
        {
            ...
            await Deployment.GetFilesFromRepo(sourceDatabaseName, true);
            string folder = "data" + Path.DirectorySeparatorChar + sourceDatabaseName;
            Container productContainer = serverlessDatabase.GetContainer("Product");
            await Deployment.LoadContainerFromFile(productContainer, folder + Path.DirectorySeparatorChar + "Product");
            Container customerContainer = serverlessDatabase.GetContainer("Customer");
            await Deployment.LoadContainerFromFile(customerContainer, folder + Path.DirectorySeparatorChar + "Customer");
            Container salesOrderContainer = serverlessDatabase.GetContainer("SalesOrder");
            await Deployment.LoadContainerFromFile(salesOrderContainer, folder + Path.DirectorySeparatorChar + "SalesOrder");
        }
        ...
        {
            Database database = serverlessClient.GetDatabase("database-v3");
        ...
        {
            Database database = serverlessClient.GetDatabase("database-v4");
            Container container = database.GetContainer("customer");
